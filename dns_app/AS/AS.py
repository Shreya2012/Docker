...
print("Socket at Server Created")
flag = False

while(True):
    print("Waiting to recieve the client request")
    ...
    if(len(data) == 4): 
        with open('.//regis_file.txt', 'w', encoding='utf-8') as f:
            if (data['TYPE'] == None or data['NAME'] == None or data['VALUE'] == None or data['TTL'] == None):
                message = "400"
                print("FS and AS not registered yet or Error in the input. Try registering again!")
            else:
